...
static int mrWriteCorners(CvCBQuad **output_quads, int count, CvSize pattern_size,
	int min_number_of_corners, std::vector<cv::Point2f> &corners);
...
// MAIN FUNCTION
//===========================================================================
int CornerFinder::FindCornersForOmniCamera(const cv::Mat& img, const cv::Size &pattern_size, std::vector<cv::Point2f> &chessbord_corners)
{
	IplImage img_input = img; 
	int min_number_of_corners = pattern_size.width*pattern_size.height;
	//START TIMER
#if TIMER
	...
	CvMat stub, *img = (CvMat*)&img_input;
	...
	{
		cout << "Only 8-bit grayscale or color images are supported" << endl;
		return -1;
	}
	if (pattern_size.width < 2 || pattern_size.height < 2)
	{
		cout << "Pattern should have at least 2x2 size" << endl;
		return -1;
	}
	if (pattern_size.width > 127 || pattern_size.height > 127)
	{
		cout << "Pattern should not have a size larger than 127 x 127" << endl;
		return -1;
	}
	...
	cout << "Time 0.1 for cvFindChessboardCorners2 was " << time0_1 << " seconds." << endl;
#endif
	...
	for (dilations = min_dilations; dilations <= max_dilations; dilations++)
	{
		...
		cout << "Time 0.2 for cvFindChessboardCorners2 was " << time0_2 << " seconds." << endl;
#endif
		...
		cout << "Time 0.3 for cvFindChessboardCorners2 was " << time0_3 << " seconds." << endl;
#endif
		...
		cout << "Time 0.4 for cvFindChessboardCorners2 was " << time0_4 << " seconds." << endl;
#endif
	...
	cout << "Time 1 for cvFindChessboardCorners2 was " << time1 << " seconds." << endl;
#endif
	// If enough corners have been found already, then there is no need for PART 2 ->EXIT
	found = mrWriteCorners(output_quad_group, max_count, pattern_size, min_number_of_corners, chessbord_corners);
	if (found == -1 || found == 1)
		EXIT;
	...
	*/
	...
	if (found == -1)
	{
		cout << "While linking the corners a problem was encountered. No corner sequence is returned. " << endl;
		return -1;
	}
	...
	cout << "Time 3 for cvFindChessboardCorners2 was " << time3 << " seconds." << endl;
#endif
...
#if TIMER
	float time = (float)(clock() - start_time) / CLOCKS_PER_SEC;
	cout << "Time for mrFindQuadNeighbors2 was " << time << " seconds." << endl;
#endif
}
...
#if TIMER
	float time = (float)(clock() - start_time) / CLOCKS_PER_SEC;
	cout << "Time for mrAugmentBestRun was " << time << " seconds." << endl;
#endif
...
#if TIMER
	float time = (float)(clock() - start_time) / CLOCKS_PER_SEC;
	cout << "Time for icvGenerateQuads was " << time << " seconds." << endl;
#endif
...
//===========================================================================
static int mrWriteCorners(CvCBQuad **output_quads, int count, CvSize pattern_size, int min_number_of_corners, std::vector<cv::Point2f> &corners)
{
	// Initialize
	int corner_count = 0;
	bool flagRow = false;
				...
				for (int l = 0; l < 4; l++)
				{
					if (((output_quads[k])->corners[l]->row == i) && ((output_quads[k])->corners[l]->column == j))
					{
						...
						if (iter == 2)
						{
							...
							corners.push_back((output_quads[k])->corners[l]->pt);
	...
	cv::Size size(maxPattern_sizeRow, maxPattern_sizeColumn);
